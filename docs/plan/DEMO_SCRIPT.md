...
## 큐레이션 capture (잠정)
`260612_office_backup/206/beef/striploin/251016` (posIdx 1, 10밴드 — AI 5ch 게이트 가능).
⚠️ **Phase-0 육안검증 게이트**: 실제 overlay에서 **비자명·서사일관 ROI 불일치**(Rule-only=모폴로지/트레이존 아티팩트, ONNX-only=AI가 실제 고기 회복)가 보여야 한다. near-perfect overlap이면 hero가 비므로 다른 capture로 교체.
